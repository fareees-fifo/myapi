    ...
    try:
        ...
        if sys_info.get("error"):
            error_message = (
                ...
                f"{BULLET_LINK} 𝐁𝐨𝐭 𝐁𝐲 {ARROW_RIGHT} <a href='tg://resolve?domain=farxxes'>kคli liຖนxx</a>\n"
            )
        ...
        status_message += f"\n{SECTION_DIVIDER}\n{BULLET_LINK} <b>𝐁𝐨𝐭 𝐁𝐲</b> {ARROW_RIGHT} <a href='tg://resolve?domain=farxxes'>kคli liຖนxx</a>"
...
async def refresh_status_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Add bot info at the end
    status_message += f"\n{SECTION_DIVIDER}\n{BULLET_LINK} <b>𝐁𝐨𝐭 𝐁𝐲</b> {ARROW_RIGHT} <a href='tg://resolve?domain=farxxes'>faressss</a>"
